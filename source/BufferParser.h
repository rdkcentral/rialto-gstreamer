...
private:
    ...
    parseSpecificPartOfBuffer(GstBuffer *buffer, int streamId, GstStructure *structure, int64_t timeStamp, int64_t duration) = 0;
...
private:
    std::unique_ptr<firebolt::rialto::IMediaPipeline::MediaSegment>
    parseSpecificPartOfBuffer(GstBuffer *buffer, int streamId, GstStructure *structure, int64_t timeStamp, int64_t duration) override;
};
